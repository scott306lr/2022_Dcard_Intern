...
const RepoInfo: NextPage = () => {
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-bg_color">
      ...
      <main
        className={`${
          status === 'ok' ? 'flex-center' : 'hidden'
        } flex-col h-full text-gray-300`}
      >
        <div className="flex-center flex-col w-full max-w-[40rem] px-5">
          <div className="flex-center w-full">
            <p className="inline truncate text-xl">
              {' '}
          ...
          >
            <div className="flex w-full space-x-4 py-2 truncate">
              <h1 className="text-2xl inline truncate" id="full_name">
                {userRepo?.full_name}
              </h1>
              <span className="flex-center hidden sm:flex text-xs px-2 border border-gray-300 rounded-full">
                {userRepo?.private ? 'Private' : 'Public'}
              </span>
              <span
            ...
            {userRepo?.description ? (
              <p id="description" className="flex text-md">
                {' '}
                {userRepo?.description}{' '}
              </p>
            ) : (
              <p id="description" className="flex text-md text-gray-500">
                {' '}
