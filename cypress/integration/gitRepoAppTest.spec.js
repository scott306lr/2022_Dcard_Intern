describe('GitRepo App Tests', () => {
  ...
  it('repository list - includes "name", "stargazers_count"', () => {
    ...
    cy.get('ul').find('li').first().find('span[id=star-count] > p').should('exist');
  });

  it('repository list - redirects correctly after click', () => {
    cy.visit('/users/scott306lr/repos');
    cy.get('ul').find('li').first().find('h1').then(($li) => {
      const text = $li.text();
      cy.wrap($li).click();
  ...
  it('single repository page - includes "full_name", "description", "stargazers_count"', () => {
    ...
    cy.get('h1[id=full_name]').should('include.text', 'scott306lr/2022_Dcard_Intern');
    //repository.description
    cy.get('p[id=description]').should('include.text', '2022 Web Frontend Intern Homework');
    //repository.stargazers_count
  ...
  it('single repository page - contains href to githubpage', () => {
    cy.visit('/users/scott306lr/repos/2022_Dcard_Intern');
    cy.get('a[href="https://github.com/scott306lr/2022_Dcard_Intern"]').should('exist');
  });
